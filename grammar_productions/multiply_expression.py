...
class MultiplyExpression(Production):
    ...
    def __repr__(self):
        return f'MultiplyExpression({self.left_element}, {self.right_element})'
    ...
    def interpret(self, world_state):
        if type(self.left_element) == Integer:
            multiply = self.left_element.interpret(world_state) * self.right_element.interpret(world_state)
            print(multiply)
            return multiply
        else:
