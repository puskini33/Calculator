...
class DivideExpression(Production):
    ...
    def __repr__(self):
        return f'DivideExpression({self.left_element}, {self.right_element})'
    ...
    def interpret(self, world_state):
        if type(self.left_element) == Integer:
            division = self.left_element.interpret(world_state) / self.right_element.interpret(world_state)
            print(division)
            return division
        else:
